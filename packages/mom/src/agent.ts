import { Agent, type AgentEvent } from "@adamliang0/pi-agent-core";
import { getModel, type ImageContent } from "@adamliang0/pi-ai";
import {
  AgentSession,
  AuthStorage,
  convertToLlm,
...
function formatToolArgsForSlack(
  _toolName: string,
...
function createRunner(
    ...
    if (event.type === "tool_execution_start") {
      ...
      if (agentEvent.message.role === "assistant") {
        ...
        if (assistantMsg.usage) {
          ...
          runState.totalUsage.cost.cacheRead +=
            assistantMsg.usage.cost.cacheRead;
          runState.totalUsage.cost.cacheWrite +=
            assistantMsg.usage.cost.cacheWrite;
      ...
      if (compEvent.result) {
        log.logInfo(
          `Auto-compaction complete: ${compEvent.result.tokensBefore} tokens compacted`,
        );
      ...
      queue.enqueue(
        () =>
          ctx.respond(
            `_Retrying (${retryEvent.attempt}/${retryEvent.maxAttempts})..._`,
            false,
          ),
        "retry",
      );
  ...
  return {
    async run(
      ...
      if (syncedCount > 0) {
        log.logInfo(
          `[${channelId}] Synced ${syncedCount} messages from log.jsonl`,
        );
      }
      ...
      log.logInfo(
        `Channels: ${ctx.channels.length}, Users: ${ctx.users.length}`,
      );
      ...
      await writeFile(
        join(channelDir, "last_prompt.jsonl"),
      ...
      if (runState.stopReason === "error" && runState.errorMessage) {
        ...
        if (
          ...
          try {
            ...
            const errMsg = err instanceof Error ? err.message : String(err);
            log.logWarning(
              "Failed to delete message for silent response",
              errMsg,
            );
          }
