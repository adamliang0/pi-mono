...
export class SlackBot {
  ...
  async start(): Promise<void> {
    ...
    log.logInfo(
      `Loaded ${this.channels.size} channels, ${this.users.size} users`,
    );
  ...
  enqueueEvent(event: SlackEvent): boolean {
    ...
    log.logInfo(
      `Enqueueing event for ${event.channel}: ${event.text.substring(0, 50)}`,
    );
  ...
  private setupEventHandlers(): void {
    ...
    this.socketClient.on("app_mention", ({ event, ack }) => {
      ...
      if (this.handler.isRunning(e.channel)) {
        ...
        this.getQueue(e.channel).enqueue(() =>
          this.handler.handleEvent(slackEvent, this),
        );
      }
  ...
  private async fetchChannels(): Promise<void> {
    ...
    do {
      ...
      const ims = result.channels as
        | Array<{ id?: string; user?: string }>
        | undefined;
      if (ims) {
        for (const im of ims) {
          if (im.id) {
