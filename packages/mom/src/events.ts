...
export class EventsWatcher {
  ...
  start(): void {
    ...
    log.logInfo(
      `Events watcher started, tracking ${this.knownFiles.size} files`,
    );
  }
  ...
  private parseEvent(content: string, filename: string): MomEvent | null {
    ...
    if (!data.type || !data.channelId || !data.text) {
      throw new Error(
        `Missing required fields (type, channelId, text) in ${filename}`,
      );
    }
    ...
    switch (data.type) {
      ...
      case "one-shot":
        if (!data.at) {
          throw new Error(
            `Missing 'at' field for one-shot event in ${filename}`,
          );
        }
        return {
          type: "one-shot",
      ...
      case "periodic":
        if (!data.schedule) {
          throw new Error(
            `Missing 'schedule' field for periodic event in ${filename}`,
          );
        }
        if (!data.timezone) {
          throw new Error(
            `Missing 'timezone' field for periodic event in ${filename}`,
          );
        }
        return {
          type: "periodic",
  ...
  private handleOneShot(filename: string, event: OneShotEvent): void {
    ...
    log.logInfo(
      `Scheduling one-shot event: ${filename} in ${Math.round(delay / 1000)}s`,
    );
