...
export function logToolSuccess(
  ...
  console.log(
    chalk.yellow(
      `${timestamp()} ${formatContext(ctx)} ✓ ${toolName} (${duration}s)`,
    ),
  );
...
// Attachments
export function logDownloadStart(
  ctx: LogContext,
  filename: string,
  localPath: string,
): void {
  console.log(
    chalk.yellow(
      `${timestamp()} ${formatContext(ctx)} ↓ Downloading attachment`,
    ),
  );
...
export function logDownloadError(
  ...
  console.log(
    chalk.yellow(`${timestamp()} ${formatContext(ctx)} ✗ Download failed`),
  );
...
export function logStopRequest(ctx: LogContext): void {
  ...
  console.log(
    chalk.yellow(
      `${timestamp()} ${formatContext(ctx)} ⊗ Stop requested - aborting`,
    ),
  );
}
