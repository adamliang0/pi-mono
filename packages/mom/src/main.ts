...
if (!parsedArgs.workingDir) {
  console.error(
    "Usage: mom [--sandbox=host|docker:<name>] <working-directory>",
  );
...
function createSlackContext(
  ...
  const eventFilename = isEvent
    ? event.text.match(/^\[EVENT:([^:]+):/)?.[1]
    : undefined;
          ...
          if (accumulatedText.length > MAX_MAIN_LENGTH) {
            accumulatedText =
              accumulatedText.substring(
                0,
                MAX_MAIN_LENGTH - truncationNote.length,
              ) + truncationNote;
          }
      ...
      updatePromise = updatePromise.then(async () => {
        try {
          ...
          if (text.length > MAX_MAIN_LENGTH) {
            accumulatedText =
              text.substring(0, MAX_MAIN_LENGTH - truncationNote.length) +
              truncationNote;
          } else {
...
const handler: MomHandler = {
  ...
  async handleEvent(
    ...
    log.logInfo(
      `[${event.channel}] Starting run: ${event.text.substring(0, 50)}`,
    );
