...
export const startModel = async (
  ...
  if (pod.models[name]) {
    console.error(
      chalk.red(`Model '${name}' already exists on pod '${podName}'`),
    );
    process.exit(1);
  }
  ...
  if (options.vllmArgs?.length) {
    ...
    console.log(
      chalk.gray("Using custom vLLM args, GPU allocation managed by vLLM"),
    );
    ...
    if (options.gpus) {
      ...
      if (options.gpus > pod.gpus.length) {
        console.error(
          chalk.red(
            `Error: Requested ${options.gpus} GPUs but pod only has ${pod.gpus.length}`,
          ),
        );
        process.exit(1);
      }
      ...
      if (!modelConfig) {
        console.error(
          chalk.red(
            `Model '${getModelName(modelId)}' not compatible with this pod's GPUs`,
          ),
        );
        process.exit(1);
      }
    ...
    if (options.gpus) {
      console.error(
        chalk.red("Error: --gpus can only be used with predefined models"),
      );
      console.error(
        chalk.yellow(
  ...
  if (!options.vllmArgs?.length) {
    if (options.memory) {
      ...
      vllmArgs = vllmArgs.filter(
        (arg) => !arg.includes("gpu-memory-utilization"),
      );
    ...
    if (options.context) {
      ...
      const maxTokens =
        contextSizes[options.context.toLowerCase()] ||
...
WRAPPER
  ...
  const processOutput = (data: Buffer) => {
    ...
    for (const line of lines) {
      if (line) {
        ...
        if (
          line.includes("Model runner exiting with code") &&
          !line.includes("code 0")
        ) {
          startupFailed = true;
        ...
        if (
          line.includes("Script exited with code") &&
          !line.includes("code 0")
        ) {
          startupFailed = true;
        ...
        if (
          line.includes("torch.OutOfMemoryError") ||
          line.includes("CUDA out of memory")
        ) {
          startupFailed = true;
  ...
  if (startupFailed) {
    ...
    console.log(
      chalk.white("Base URL:    ") + chalk.yellow(`http://${host}:${port}/v1`),
    );
    console.log(chalk.white("Model:       ") + chalk.yellow(modelId));
    console.log(
      chalk.white("API Key:     ") +
        chalk.yellow(process.env.PI_API_KEY || "(not set)"),
    );
    console.log(chalk.cyan("─".repeat(50)));

    console.log(`\n${chalk.bold("Export for shell:")}`);
    console.log(
      chalk.gray(`export OPENAI_BASE_URL="http://${host}:${port}/v1"`),
    );
    console.log(
      chalk.gray(
        `export OPENAI_API_KEY="${process.env.PI_API_KEY || "your-api-key"}"`,
      ),
    );
  ...
  curl $OPENAI_BASE_URL/chat/completions \\
    ...
    console.log("");
    console.log(
      chalk.cyan(`Chat with model:  pi agent ${name} "Your message"`),
    );
...
export const stopAllModels = async (options: { pod?: string }) => {
  ...
  console.log(
    chalk.yellow(
      `Stopping ${modelNames.length} model(s) on pod '${podName}'...`,
    ),
  );
...
export const listModels = async (options: { pod?: string }) => {
  ...
  const sshParts = pod.ssh.split(" ");
  const host =
    sshParts.find((p) => p.includes("@"))?.split("@")[1] || "unknown";
    ...
    const gpuStr =
      model.gpu.length > 1
        ...
        : model.gpu.length === 1
          ...
          : "GPU unknown";
    console.log(
      `  ${chalk.green(name)} - Port ${model.port} - ${gpuStr} - PID ${model.pid}`,
    );
  ...
  for (const name of modelNames) {
    ...
    if (status === "dead") {
      ...
      console.log(
        chalk.red(
          `  ${name}: vLLM crashed (check logs with 'pi logs ${name}')`,
        ),
      );
      anyDead = true;
...
export const viewLogs = async (name: string, options: { pod?: string }) => {
  ...
  console.log(
    chalk.green(`Streaming logs for '${name}' on pod '${podName}'...`),
  );
...
export const showKnownModels = async () => {
  ...
  if (activePod) {
    ...
    console.log(
      chalk.bold(
        `Known Models for ${activePod.name} (${podGpuCount}x ${podGpuType || "GPU"}):\n`,
      ),
    );
  } else {
    ...
    console.log(
      chalk.yellow(
        "No active pod. Use 'pi pods active <name>' to filter compatible models.\n",
      ),
    );
  }
  ...
  if (activePod && Object.keys(compatible).length > 0) {
    ...
    for (const family of sortedFamilies) {
      ...
      const modelList = compatible[family].sort((a, b) =>
  ...
  if (Object.keys(incompatible).length > 0) {
    if (activePod && Object.keys(compatible).length > 0) {
      console.log(
        chalk.red.bold("✗ Incompatible Models (need more/different GPUs):\n"),
      );
    }
    ...
    for (const family of sortedFamilies) {
      ...
      const modelList = incompatible[family].sort((a, b) =>
        a.name.localeCompare(b.name),
      );
