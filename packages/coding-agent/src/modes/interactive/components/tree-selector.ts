...
class TreeList implements Component {
  ...
  private applyFilter(): void {
    ...
    if (this.foldedNodes.size > 0) {
      ...
      for (const flatNode of this.flatNodes) {
        ...
        if (
          parentId != null &&
          (this.foldedNodes.has(parentId) || skipSet.has(parentId))
        ) {
          skipSet.add(id);
        }
      }
  ...
  render(width: number): string[] {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const showsFoldInConnector =
        flatNode.showConnector && !flatNode.isVirtualRootChild;
      const foldMarker =
        isFolded && !showsFoldInConnector ? theme.fg("accent", "⊞ ") : "";
  ...
  handleInput(keyData: string): void {
    ...
    if (kb.matches(keyData, "tui.select.up")) {
      ...
      if (
        currentId &&
        this.isFoldable(currentId) &&
        !this.foldedNodes.has(currentId)
      ) {
  ...
  private findBranchSegmentStart(direction: "up" | "down"): number {
    ...
    while (true) {
      const parentId: string | null =
        this.visibleParentMap.get(currentId) ?? null;
