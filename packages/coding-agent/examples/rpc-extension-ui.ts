...
class LoadingIndicator implements Component {
  ...
  render(_width: number): string[] {
    return [
      `${BLUE}${BOLD}Agent:${RESET} ${DIM}Working${".".repeat(this.dots)}${RESET}`,
    ];
  }
}
...
async function main() {
    ...
    switch (cmd) {
      case "/select":
        showSelectDialog(
          ...
          (value) => {
            if (value) {
              outputLog.append(
                `${MAGENTA}${BOLD}Notification:${RESET} You picked: ${value}`,
              );
            } else {
              outputLog.append(
                `${MAGENTA}${BOLD}Notification:${RESET} Selection cancelled`,
              );
            }
            tui.requestRender();
          },
      ...
      case "/confirm":
        showSelectDialog("Are you sure?", ["Yes", "No"], (value) => {
          ...
          outputLog.append(
            `${MAGENTA}${BOLD}Notification:${RESET} Confirmed: ${confirmed}`,
          );
          tui.requestRender();
        });
        return true;
      ...
      case "/input":
        showInputDialog("Enter your name", undefined, (value) => {
          if (value) {
            outputLog.append(
              `${MAGENTA}${BOLD}Notification:${RESET} You entered: ${value}`,
            );
          } else {
            outputLog.append(
              `${MAGENTA}${BOLD}Notification:${RESET} Input cancelled`,
            );
          }
          tui.requestRender();
        });
      ...
      case "/editor":
        showInputDialog("Edit text", "Hello, world!", (value) => {
          if (value) {
            outputLog.append(
              `${MAGENTA}${BOLD}Notification:${RESET} Submitted: ${value}`,
            );
          } else {
            outputLog.append(
              `${MAGENTA}${BOLD}Notification:${RESET} Editor cancelled`,
            );
          }
          tui.requestRender();
        });
  ...
  outputLog.append(
    `${DIM}Slash commands: /select /confirm /input /editor${RESET}`,
  );
  outputLog.append("");
