    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ...
      >(
        (_tui, theme, _keybindings, done) =>
          new OverlayTestComponent(theme, done),
        { overlay: true },
      );

      if (result) {
        const msg = result.query
          ? `${result.action}: "${result.query}"`
          : result.action;
...
class OverlayTestComponent implements Focusable {
  ...
  constructor(
    private theme: Theme,
    private done: (
      result: { action: string; query?: string } | undefined,
    ) => void,
  ) {}
  ...
  handleInput(data: string): void {
    ...
    if (matchesKey(data, "return")) {
      this.done({
        ...
        query: current.hasInput ? current.text : undefined,
      });
      return;
    }
    ...
    if (matchesKey(data, "up")) {
      ...
      if (matchesKey(data, "backspace")) {
        if (current.cursor > 0) {
          current.text =
            current.text.slice(0, current.cursor - 1) +
        ...
        current.text =
          current.text.slice(0, current.cursor) +
          data +
          current.text.slice(current.cursor);
        current.cursor++;
      }
  ...
  render(_width: number): string[] {
    ...
    lines.push(
      row(` ${th.fg("dim", "─── Edge Cases (borders should align) ───")}`),
    );
    lines.push(
      row(
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (item.hasInput) {
        ...
        if (isSelected) {
          ...
          const cursorChar =
            item.cursor < inputDisplay.length ? inputDisplay[item.cursor] : " ";
    ...
    lines.push(
      row(
        ` ${th.fg("dim", "↑↓ navigate • type to input • Enter select • Esc cancel")}`,
      ),
    );
    lines.push(th.fg("border", `╰${"─".repeat(innerW)}╯`));
