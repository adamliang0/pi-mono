...
class SpaceInvadersComponent {
  ...
  private tick(): void {
    ...
    if (this.keys.fire && this.fireCooldown === 0) {
      const playerBullets = this.state.bullets.filter(
        (b) => b.direction === -1,
      );
  ...
  private alienShoot(): void {
    ...
    if (
      shooters.length > 0 &&
      this.state.bullets.filter((b) => b.direction === 1).length < 3
    ) {
  ...
  private checkCollisions(): void {
    ...
    for (const bullet of this.state.bullets) {
      ...
      if (bullet.direction === 1) {
        if (
          Math.abs(bullet.x - this.state.player.x) <= 1 &&
          bullet.y === PLAYER_Y
        ) {
          bulletsToRemove.add(bullet);
    ...
    for (let y = 0; y < GAME_HEIGHT; y++) {
      ...
      for (let x = 0; x < GAME_WIDTH; x++) {
        ...
        if (
          !colored &&
          y === PLAYER_Y &&
          Math.abs(x - this.state.player.x) <= 1
        ) {
  ...
  pi.registerCommand("invaders", {
    ...
    handler: async (_args, ctx) => {
      ...
      for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (
          entry.type === "custom" &&
          entry.customType === INVADERS_SAVE_TYPE
        ) {
