...
async function getDirectAccessToken(
  ...
  const response = await fetch(
    `${GITLAB_COM_URL}/api/v4/ai/third_party_agents/direct_access`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to get direct access token: ${response.status} ${errorText}`,
    );
  }
  ...
  (async () => {
    try {
      ...
      if (!gitlabAccessToken)
        throw new Error(
          "No GitLab access token. Run /login gitlab-duo or set GITLAB_TOKEN",
        );
