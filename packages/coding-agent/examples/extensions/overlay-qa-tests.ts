...
import { matchesKey, truncateToWidth, visibleWidth } from "@adamliang0/pi-tui";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("overlay-animation", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new AnimationDemoComponent(tui, theme, done),
        {
          overlay: true,
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ...
      while (true) {
        const result = await ctx.ui.custom<"next" | "confirm" | "cancel">(
          (_tui, theme, _kb, done) =>
            new AnchorTestComponent(theme, anchors[index]!, done),
          {
            overlay: true,
  ...
  pi.registerCommand("overlay-margins", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      const configs: { name: string; options: OverlayOptions }[] = [
        {
          name: "No margin (top-left)",
          options: { anchor: "top-left", width: 35 },
        },
        {
          name: "Margin: 3 all sides",
          options: { anchor: "top-left", width: 35, margin: 3 },
        },
        {
      ...
      while (true) {
        const result = await ctx.ui.custom<"next" | "close">(
          (_tui, theme, _kb, done) =>
            new MarginTestComponent(theme, configs[index]!, done),
          {
            overlay: true,
  ...
  pi.registerCommand("overlay-stack", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ...
      const p1 = ctx.ui.custom<string>(
        (_tui, theme, _kb, done) =>
          new StackOverlayComponent(theme, 1, "back (red border)", done),
        {
          overlay: true,
          overlayOptions: {
            anchor: "center",
      ...
      const p2 = ctx.ui.custom<string>(
        (_tui, theme, _kb, done) =>
          new StackOverlayComponent(theme, 2, "middle (green border)", done),
        {
          overlay: true,
          overlayOptions: {
            anchor: "center",
      ...
      const p3 = ctx.ui.custom<string>(
        (_tui, theme, _kb, done) =>
          new StackOverlayComponent(theme, 3, "front (blue border)", done),
        {
          overlay: true,
          overlayOptions: {
            anchor: "center",
  ...
  pi.registerCommand("overlay-overflow", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) =>
          new StreamingOverflowComponent(tui, theme, done),
        {
          overlay: true,
  ...
  pi.registerCommand("overlay-edge", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (_tui, theme, _kb, done) => new EdgeTestComponent(theme, done),
        {
          overlay: true,
          overlayOptions: {
            anchor: "right-center",
  ...
  pi.registerCommand("overlay-percent", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ...
      while (true) {
        ...
        const result = await ctx.ui.custom<"next" | "close">(
          (_tui, theme, _kb, done) =>
            new PercentTestComponent(theme, config, done),
          {
            overlay: true,
            overlayOptions: {
              width: 30,
  ...
  pi.registerCommand("overlay-maxheight", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (_tui, theme, _kb, done) => new MaxHeightTestComponent(theme, done),
        {
          overlay: true,
  ...
  pi.registerCommand("overlay-sidepanel", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new SidepanelComponent(tui, theme, done),
        {
          overlay: true,
          overlayOptions: {
            anchor: "right-center",
  ...
  pi.registerCommand("overlay-toggle", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new ToggleDemoComponent(tui, theme, done),
        {
          overlay: true,
  ...
  pi.registerCommand("overlay-passive", {
    description:
      "Test non-capturing overlay (passive info panel alongside active overlay)",
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ctx.ui.setEditorText("");
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new PassiveDemoController(tui, theme, done),
        {
          overlay: true,
  ...
  pi.registerCommand("overlay-focus", {
    description:
      "Test focus cycling and rendering order with non-capturing overlays",
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ctx.ui.setEditorText("");
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new FocusDemoController(tui, theme, done),
        {
          overlay: true,
          overlayOptions: {
  ...
  pi.registerCommand("overlay-streaming", {
    description:
      "Multiple input panels with simulated streaming (Tab to cycle focus)",
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ctx.ui.setEditorText("");
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) =>
          new StreamingInputController(tui, theme, done),
        {
          overlay: true,
          overlayOptions: {
            anchor: "bottom-center",
...
class StackOverlayComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      border("│") +
        padLine(` Overlay ${th.fg("accent", `#${this.num}`)}`) +
        border("│"),
    );
    lines.push(
      border("│") +
        padLine(` Layer: ${th.fg(color, this.position)}`) +
        border("│"),
    );
    ...
    for (let i = 0; i < 5; i++) {
      lines.push(
        border("│") + padLine(` ${"░".repeat(innerW - 2)} `) + border("│"),
      );
    }
    lines.push(border("│") + padLine("") + border("│"));
    lines.push(
      border("│") +
        padLine(th.fg("dim", " Press Enter/Esc to close")) +
        border("│"),
    );
    lines.push(border(`╰${"─".repeat(innerW)}╯`));
...
class StreamingOverflowComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    const title = truncateToWidth(
      ` Streaming Output (${this.lines.length} lines) `,
      innerW,
    );
    const titlePad = Math.max(0, innerW - visibleWidth(title));
    result.push(
      border("╭") + th.fg("accent", title) + border(`${"─".repeat(titlePad)}╮`),
    );

    // Scroll indicators
    const canScrollUp = this.scrollOffset > 0;
    const canScrollDown =
      this.scrollOffset < this.lines.length - this.maxVisibleLines;
    const scrollInfo = `↑${this.scrollOffset} | ↓${Math.max(0, this.lines.length - this.maxVisibleLines - this.scrollOffset)}`;

    result.push(
      border("│") +
        padLine(
          canScrollUp || canScrollDown ? th.fg("dim", ` ${scrollInfo}`) : "",
        ) +
        border("│"),
    );
    ...
    const status = this.finished
      ? th.fg("success", "✓ Done")
      : th.fg("warning", "● Running");
    result.push(
      border("│") +
        padLine(` ${status} ${th.fg("dim", "| ↑↓ scroll | Esc close")}`) +
        border("│"),
    );
...
class SidepanelComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      border("│") +
        padLine(th.fg("accent", " Responsive Sidepanel")) +
        border("│"),
    );
    ...
    lines.push(
      border("│") +
        padLine(th.fg("warning", " ⚠ Resize terminal < 100 cols")) +
        border("│"),
    );
    lines.push(
      border("│") +
    ...
    lines.push(
      border("│") +
        padLine(th.fg("dim", " Uses visible: (w) => w >= 100")) +
        border("│"),
    );
    lines.push(
      border("│") +
        padLine(th.fg("dim", " ↑↓ navigate | Esc close")) +
        border("│"),
    );
    lines.push(border(`╰${"─".repeat(innerW)}╯`));
...
class AnimationDemoComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      border("│") +
        padLine(th.fg("accent", " Animation Demo (~30 FPS)")) +
        border("│"),
    );
    lines.push(border("│") + padLine(``) + border("│"));
    lines.push(
      border("│") +
        padLine(` Frame: ${th.fg("accent", String(this.frame))}`) +
        border("│"),
    );
    lines.push(
      border("│") +
        padLine(` FPS: ${th.fg("success", String(this.fps))}`) +
        border("│"),
    );
    lines.push(border("│") + padLine(``) + border("│"));
    ...
    const pos = Math.max(
      0,
      Math.floor(((Math.sin(this.frame / 10) + 1) * (barWidth - 10)) / 2),
    );
    const bar =
      " ".repeat(pos) +
      th.fg("accent", "██████████") +
      " ".repeat(Math.max(0, barWidth - 10 - pos));
    ...
    lines.push(
      border("│") +
        padLine(` Spinner: ${th.fg("warning", spin!)}`) +
        border("│"),
    );
    ...
    lines.push(
      border("│") +
        padLine(th.fg("dim", " This proves overlays can handle")) +
        border("│"),
    );
    lines.push(
      border("│") +
        padLine(th.fg("dim", " real-time game-like rendering.")) +
        border("│"),
    );
    lines.push(
      border("│") +
        padLine(th.fg("dim", " (pi-doom uses same approach)")) +
        border("│"),
    );
    lines.push(border("│") + padLine(``) + border("│"));
    lines.push(
      border("│") + padLine(th.fg("dim", " Press Esc to close")) + border("│"),
    );
    lines.push(border(`╰${"─".repeat(innerW)}╯`));
...
class PassiveDemoController extends BaseOverlay {
  ...
  render(width: number): string[] {
    const th = this.theme;
    const display =
      this.typed.length > 0 ? this.typed : th.fg("dim", "(type here)");
    return this.box(
      [
        "",
...
class TimerPanel extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    return this.box(
      [` ${th.fg("accent", time)}`, th.fg("dim", " nonCapturing: true")],
      width,
      "Timer",
    );
  }
...
class FocusPanel extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(border(`╭${"─".repeat(innerW)}╮`));
    lines.push(
      border("│") + padLine(` ${th.fg("accent", this.label)}`) + border("│"),
    );
    lines.push(border("│") + padLine("") + border("│"));
    if (focused) {
      lines.push(
        border("│") + padLine(th.fg("success", " ● FOCUSED")) + border("│"),
      );
      lines.push(
        border("│") + padLine(th.fg("dim", " (receiving input)")) + border("│"),
      );
    } else {
      lines.push(
        border("│") + padLine(th.fg("dim", " ○ unfocused")) + border("│"),
      );
      lines.push(
        border("│") + padLine(th.fg("dim", " (passive)")) + border("│"),
      );
    }
    lines.push(border("│") + padLine("") + border("│"));
    lines.push(border(`╰${"─".repeat(innerW)}╯`));
...
class StreamingInputController extends BaseOverlay {
  ...
  constructor(
    ...
    this.streamInterval = setInterval(() => {
      ...
      this.streamLines.push(
        `[${timestamp}] Streaming line ${this.lineCount}...`,
      );
  ...
  render(width: number): string[] {
    ...
    lines.push("");
    lines.push(
      ` Three ${th.fg("accent", "nonCapturing")} input panels on the left.`,
    );
...
class StreamingInputPanel implements Component {
  ...
  render(width: number): string[] {
    ...
    const inputDisplay =
      this.typed.length > 0 ? this.typed : th.fg("dim", "(type here)");
    const truncatedInput = truncateToWidth(
      ` > ${inputDisplay}`,
      innerW,
      "...",
      true,
