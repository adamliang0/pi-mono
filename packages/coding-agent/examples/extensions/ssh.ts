...
function createRemoteReadOps(
  ...
  return {
    ...
    access: (p) =>
      sshExec(remote, `test -r ${JSON.stringify(toRemote(p))}`).then(() => {}),
...
function createRemoteWriteOps(
  ...
  return {
    ...
    mkdir: (dir) =>
      sshExec(remote, `mkdir -p ${JSON.stringify(toRemote(dir))}`).then(
        () => {},
      ),
  };
}
  ...
  return {
    exec: (command, cwd, { onData, signal, timeout }) =>
      new Promise((resolve, reject) => {
        ...
        const child = spawn("ssh", [remote, cmd], {
          stdio: ["ignore", "pipe", "pipe"],
        });
        let timedOut = false;
        const timer = timeout
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (arg) {
      ...
      ctx.ui.setStatus(
        ...
        ctx.ui.theme.fg(
          "accent",
          `SSH: ${resolvedSsh.remote}:${resolvedSsh.remoteCwd}`,
        ),
      );
      ctx.ui.notify(
        `SSH mode: ${resolvedSsh.remote}:${resolvedSsh.remoteCwd}`,
        "info",
      );
    }
  });
