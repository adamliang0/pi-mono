...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const isDangerous =
      /\brm\s+(-rf?|--recursive)/i.test(command) || /\bsudo\b/i.test(command);
    ...
    if (isDangerous) {
      ...
      const choice = await ctx.ui.select(`Dangerous command: ${command}`, [
        "Allow",
        "Block",
      ]);
      if (choice !== "Allow") {
  ...
  pi.registerCommand("rpc-prefill", {
    description:
      "Prefill the input editor (demonstrates ctx.ui.setEditorText in RPC)",
    handler: async (_args, ctx) => {
