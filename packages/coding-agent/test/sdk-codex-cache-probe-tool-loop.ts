...
import { type AssistantMessage, getModel, Type } from "@adamliang0/pi-ai";
...
function parseArgs(argv: string[]): Args {
  ...
  if (!Number.isInteger(turns) || turns < MIN_TURNS || turns > MAX_TURNS) {
    throw new Error(
      `--turns must be an integer between ${MIN_TURNS} and ${MAX_TURNS}`,
    );
  }
...
function buildPrompt(turn: number): string {
  ...
  lines.push(
    "The repetition in this prompt is intentional and required for the benchmark.",
  );
  ...
  lines.push(
    `After the tool result arrives, reply with exactly one line in this format:`,
  );
  ...
  lines.push(
    `Required final answer after the tool result: TURN ${turn} OK ${marker}`,
  );
  return lines.join("\n");
}
...
function deterministicProbeTool(): ToolDefinition<
  typeof deterministicProbeParameters
> {
  return {
...
async function main(): Promise<void> {
  ...
  console.log(
    `turns ${args.turns}, transport ${args.transport}, reasoning medium, maxTokens ${args.maxTokens}`,
  );
  console.log("");
  ...
  console.log("");
  console.log(
    `subrequest cache read monotonic: ${violations.length === 0 ? "yes" : "NO"}`,
  );
