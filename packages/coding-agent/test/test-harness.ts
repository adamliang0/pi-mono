...
import { createAssistantMessageEventStream } from "@adamliang0/pi-ai";
import {
  AgentSession,
...
function buildUsage(partial?: Partial<Usage>): Usage {
  ...
  return {
    ...
    cacheWrite,
    totalTokens:
      partial?.totalTokens ?? input + output + cacheRead + cacheWrite,
    cost: partial?.cost ?? {
      input: 0,
      output: 0,
      cacheRead: 0,
...
export interface HarnessOptions {
  ...
  extensionFactories?: Array<
    ExtensionFactory | CreateTestExtensionsResultInput
  >;
}
...
function createTempDir(): string {
  const tempDir = join(
    tmpdir(),
    `pi-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
function createHarnessWithResourceLoader(
  ...
  const { streamFn, state: fauxState } = createFauxStreamFn(
  ...
  return {
    ...
    eventsOfType<T extends AgentSessionEvent["type"]>(type: T) {
      return events.filter(
        (e): e is Extract<AgentSessionEvent, { type: T }> => e.type === type,
      );
    },
    tempDir,
    cleanup,
...
export async function createHarnessWithExtensions(
  options: HarnessOptions = {},
): Promise<Harness> {
  const tempDir = createTempDir();
  ...
  const resourceLoader =
    options.resourceLoader ?? createTestResourceLoader({ extensionsResult });
