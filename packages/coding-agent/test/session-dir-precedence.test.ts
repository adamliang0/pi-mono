...
const mocks = vi.hoisted(() => ({
  ...
  createAgentSession: vi.fn(
    async (options: { sessionManager?: { getSessionDir(): string } }) => {
    ...
    DefaultResourceLoader: class {
      ...
      getExtensions() {
        ...
        if (mocks.state.hookSessionDir) {
          handlers.set("session_directory", [
            async () => ({ sessionDir: mocks.state.hookSessionDir }),
          ]);
        }
        return {
          extensions: [
            { path: "/mock-extension.ts", handlers, flags: new Map() },
          ],
          errors: [],
          runtime: {
...
describe("sessionDir precedence", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = join(
      tmpdir(),
      `pi-session-dir-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it("uses settings sessionDir ahead of the session_directory hook for --resume", async () => {
    ...
    mocks.selectSession.mockImplementation(
      async (listCurrent: (onProgress: () => void) => Promise<unknown>) => {
        await listCurrent(() => {});
