...
Content`,
    ...
    it("should resolve directory with package.json pi.extensions in extensions setting", async () => {
      ...
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "helper.ts")),
      ).toBe(false);
    });
  });
  ...
  describe(".agents/skills auto-discovery", () => {
    ...
    it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
      ...
      try {
        ...
        mkdirSync(join(tempDir, ".agents", "skills", "home-skill"), {
          recursive: true,
        });
        writeFileSync(
          homeSkill,
  ...
  describe("resolveExtensionSources", () => {
    ...
    it("should handle directories with auto-discovery layout", async () => {
      ...
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "main.ts") && r.enabled,
        ),
      ).toBe(true);
      expect(
        result.themes.some(
          (r) => pathEndsWith(r.path, "dark.json") && r.enabled,
        ),
      ).toBe(true);
    });
  });
  ...
  describe("npmCommand", () => {
    ...
    it("should use npmCommand argv for npm root lookup and invalidate cached root when npmCommand changes", () => {
      ...
      expect(
        packageManager.getInstalledPath("npm:@scope/pkg", "user"),
      ).toBeUndefined();
  ...
  describe("pattern filtering in pi manifest", () => {
    it("should support glob patterns in manifest extensions", async () => {
      ...
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "skip.ts")),
      ).toBe(false);
    });
  ...
  describe("pattern filtering in package filters", () => {
    it("should apply user filters on top of manifest filters (not replace)", async () => {
      ...
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "baz.ts")),
      ).toBe(false);
    });
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
      ...
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "subagent/index.ts") && r.enabled,
        ),
      ).toBe(true);
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "standalone.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find agents.ts as a standalone extension
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "agents.ts")),
      ).toBe(false);
    });
    ...
    it("should respect package.json pi.extensions manifest in subdirectories", async () => {
      ...
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "custom/main.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find utils.ts (not declared in manifest)
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "utils.ts")),
      ).toBe(false);
    });
    ...
    it("should handle mixed top-level files and subdirectories", async () => {
      ...
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "simple.ts") && r.enabled,
        ),
      ).toBe(true);
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "complex/index.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find helper modules
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "complex/a.ts")),
      ).toBe(false);
      expect(
        result.extensions.some((r) => pathEndsWith(r.path, "complex/b.ts")),
      ).toBe(false);
    ...
    it("should skip subdirectories without index.ts or manifest", async () => {
      ...
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "valid.ts") && r.enabled,
        ),
      ).toBe(true);
  ...
  describe("offline mode and network timeouts", () => {
    ...
    it("should suggest git source prefixes for update lookups", async () => {
      ...
      await expect(
        packageManager.update("github.com/example/repo"),
      ).rejects.toThrow(
    ...
    it("should skip refreshing temporary git sources when offline", async () => {
      ...
      expect(
        result.extensions.some(
          (r) => pathEndsWith(r.path, "extensions/index.ts") && r.enabled,
        ),
      ).toBe(true);
    ...
    it("should skip pinned packages when checking for updates", async () => {
      ...
      const parsedGitSource = (packageManager as any).parseSource(
