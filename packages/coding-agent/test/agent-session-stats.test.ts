...
import { type AssistantMessage, getModel, type Usage } from "@adamliang0/pi-ai";
...
describe("AgentSession.getSessionStats", () => {
  ...
  it("reports unknown current context usage immediately after compaction", () => {
    ...
    try {
      ...
      sessionManager.appendMessage(
        createAssistantMessage("response1", 180_000, 2),
      );
      const keptUserId = sessionManager.appendMessage(
        createUserMessage("second", 3),
      );
      sessionManager.appendMessage(
        createAssistantMessage("response2", 195_000, 4),
      );
  ...
  it("uses post-compaction usage for current context instead of stale kept usage", () => {
    ...
    try {
      ...
      sessionManager.appendMessage(createUserMessage("third", 5));
      sessionManager.appendMessage(
        createAssistantMessage("response3", 25_000, 6),
      );
      syncAgentMessages(session, sessionManager);
      ...
      expect(stats.contextUsage?.percent).toBe(
        (25_000 / model.contextWindow) * 100,
      );
    } finally {
      session.dispose();
    }
