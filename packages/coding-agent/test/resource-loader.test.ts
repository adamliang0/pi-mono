  ...
  describe("override functions", () => {
    it("should apply skillsOverride", async () => {
      const injectedSkill: Skill = {
        ...
        filePath: "/fake/path",
        baseDir: "/fake",
        sourceInfo: createSyntheticSourceInfo("/fake/path", {
          source: "custom",
        }),
  ...
  pi.registerCommand("deploy", {
    ...
    handler: async () => {},
      ...
      const authStorage = AuthStorage.create(
        join(tempDir, "auth-explicit.json"),
      );
      const modelRegistry = new ModelRegistry(authStorage);
      const runner = new ExtensionRunner(
        extensionsResult.extensions,
      ...
      expect(runner.getCommand("deploy:1")?.description).toBe(
        "explicit command",
      );
      expect(runner.getCommand("deploy:2")?.description).toBe("global command");
      expect(runner.getToolDefinition("duplicate-tool")?.description).toBe(
        "explicit tool",
      );
    });
  });
