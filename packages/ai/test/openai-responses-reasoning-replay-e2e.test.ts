...
describe.skipIf(!process.env.OPENAI_API_KEY || !process.env.ANTHROPIC_API_KEY)(
  ...
  () => {
    it("skips reasoning-only history after an aborted turn", {
      retry: 2,
    }, async () => {
    ...
    it("handles same-provider different-model handoff with tool calls", {
      retry: 2,
    }, async () => {
    ...
    it("handles cross-provider handoff from Anthropic to OpenAI Codex", {
      retry: 2,
    }, async () => {
