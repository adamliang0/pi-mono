...
async function expectThinkingDisabledE2E<TApi extends Api>(
  ...
  expect(countPongs(result.text)).toBeGreaterThanOrEqual(
    expectations.minPongs ?? 35,
  );
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY)(
  ...
  () => {
    it("disables thinking for budget-based reasoning models", {
      retry: 2,
      timeout: 30000,
    }, async () => {
      await expectThinkingDisabledE2E(
    ...
    it("disables thinking for adaptive reasoning models", {
      retry: 2,
      timeout: 30000,
    }, async () => {
      await expectThinkingDisabledE2E(
...
describe.skipIf(!process.env.GEMINI_API_KEY)(
  ...
  () => {
    it("disables thinking for Gemini 2.5", {
      retry: 2,
      timeout: 30000,
    }, async () => {
    ...
    it("disables thinking for Gemini 3.x", {
      retry: 2,
      timeout: 30000,
    }, async () => {
      await expectThinkingDisabledE2E(
    ...
    it("does not error when thinking is off for Gemini 3.1 Pro", {
      retry: 2,
      timeout: 30000,
    }, async () => {
      await expectThinkingDisabledE2E(
  ...
  const vertexOptions = vertexApiKey
    ...
    : vertexProject && vertexLocation
      ? ({
        ...
        } satisfies SimpleOptionsWithExtras)
      : undefined;
...
describe("Google Gemini CLI thinking disable E2E", () => {
  it.skipIf(!geminiCliToken)(
    ...
    async () => {
      await expectThinkingDisabledE2E(
        getModel("google-gemini-cli", "gemini-2.5-flash"),
        {
...
describe.skipIf(!process.env.OPENAI_API_KEY)(
  ...
  () => {
    it("disables thinking for Responses reasoning models", {
      retry: 2,
      timeout: 30000,
    }, async () => {
...
describe.skipIf(!process.env.OPENROUTER_API_KEY)(
  ...
  () => {
    it("disables thinking for Qwen 3.5 reasoning models", {
      retry: 2,
      timeout: 30000,
    }, async () => {
      await expectThinkingDisabledE2E(
        getModel("openrouter", "qwen/qwen3.5-plus-02-15"),
        {
          maxOutputTokens: 100,
        },
      );
